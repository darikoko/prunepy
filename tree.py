    ...
    def build_tree(self):
        for html_element in document.getElementsByTagName("*"):
            if self.is_prune(html_element):
                leaf = Leaf(html_element, self.store)
...
class Leaf:
    ...
    def handle_event(self, event):
        ...
        print(function)
        eval(function, {"store":self.store}, {"event": event})
    ...
    def render(self, tree_scope: dict[str, any] = {}):
        ...
        for directive_name, directive_value in self.directives.items():
            ...
            if directive_name == "n-text":
                self.html_element.innerText = eval(directive_value, {"store":self.store}, tree_scope)
            elif directive_name == "n-html":
                self.html_element.innerHTML = eval(directive_value, {"store":self.store}, tree_scope)
            elif directive_name == "n-show":
                self.html_element.hidden = not eval(directive_value, {"store":self.store}, tree_scope)
            elif directive_name.startswith("n-on:"):
                event_type = directive_name.replace("n-on:", "")
                self.html_element.addEventListener(event_type, self.handle_event)
                ...
                if attribute_to_bind == "class":
                    self.html_element.setAttribute(
                        ...
                        + eval(directive_value, {"store":self.store}, tree_scope),
                    )
                else:
                    self.html_element.setAttribute(
                        attribute_to_bind, eval(directive_value, {"store":self.store}, tree_scope)
                    )
            elif directive_name == "n-for":
                splitted_for = directive_value.split(" in ")
                list_element = eval(splitted_for[1], {"store":self.store}, tree_scope)
